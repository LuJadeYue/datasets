...
dot, and a sphere with an X capped by a dot.
...
The indices correspond to: 
  ...
  6 = marked cylinder
  7 = marked sphere
- the rotation used in the rendering process, represented as a unit quaternion 
in xyzw format
...
"""
_DATA_PATH = 'https://storage.googleapis.com/gresearch/implicit-pdf/symsol.zip'
  ...
  def _info(self) -> tfds.core.DatasetInfo:
    ...
    return tfds.core.DatasetInfo(
        ...
        features=tfds.features.FeaturesDict({
            ...
            'label_shape':
                tfds.features.ClassLabel(num_classes=len(_SHAPE_NAMES)),
            'rotation':
                tfds.features.Tensor(shape=(4,), dtype=tf.float32),
            'rotations_equivalent':
                tfds.features.Tensor(shape=(None, 4), dtype=tf.float32),
        }),
        ...
        tfds.core.SplitGenerator(
            ...
            gen_kwargs={
                'dl_manager':
                    dl_manager,
                'images_path':
                    extracted_paths / 'symsol_dataset' / 'train' / 'images',
                'rotations_path':
                    extracted_paths / 'symsol_dataset' / 'train' /
                    'rotations.npz',
            }),
        tfds.core.SplitGenerator(
            ...
            gen_kwargs={
                'dl_manager':
                    dl_manager,
                'images_path':
                    extracted_paths / 'symsol_dataset' / 'test' / 'images',
                'rotations_path':
                    extracted_paths / 'symsol_dataset' / 'test' /
                    'rotations.npz',
            }),
    ]

  def _generate_examples(self, dl_manager, images_path, rotations_path):
    """Yields examples."""
    with tf.io.gfile.GFile(rotations_path, 'rb') as f:
